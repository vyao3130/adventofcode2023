...
with open(file, "r") as f:
    ...
    for line in f:
        ...
        if "Time" in line:
            line=line.split(" ")[1:]
            times=[int(i) for i in line if i.isnumeric()]
        if "Distance" in line:
            line=line.split(" ")[1:]
            distance=[int(i) for i in line if i.isnumeric()]
            races=[(times[num], distance[num]) for num in range(len(times))]
    
    solutions.append(f"{races} \n")
    # calculate times
    button_ranges = []
    for race in races:
        time, win_distance = race
